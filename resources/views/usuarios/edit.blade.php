...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $usuario->persona->nombre }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Editar usuario · {{ $usuario->email }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('usuarios.update', $usuario) }}" class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nombre" value="{{ old('nombre', $usuario->persona->nombre) }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6"
        ...
        <button type="button" @click="open = !open"
            ...
            <div>
                <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Cambiar contraseña</h2>
                <p class="text-xs text-gray-400 mt-0.5">Establecer una nueva contraseña para este usuario</p>
            </div>
        ...
        <form x-show="open" x-cloak method="POST"
              ...
              class="mt-4 space-y-4 pt-4 border-t border-gray-100 dark:border-gray-700">
            @csrf
            @method('PATCH')
