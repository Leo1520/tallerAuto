    ...
    <form method="POST" action="{{ route('servicios.update', $servicio) }}">
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                    ...
                    <select name="tipo_servicio_id"
                        ...
                        @foreach($tipos as $tipo)
                            <option value="{{ $tipo->id }}"
                                {{ old('tipo_servicio_id', $servicio->tipo_servicio_id) == $tipo->id ? 'selected' : '' }}>
                                {{ $tipo->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="precio" value="{{ old('precio', $servicio->precio) }}" required
                           min="0" step="0.01"
                           class="w-full px-4 py-2.5 bg-gray-900 border {{ $errors->has('precio') ? 'border-red-600' : 'border-gray-600' }} text-gray-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-600">
            ...
            <div>
                ...
                <textarea name="descripcion" rows="3"
                          class="w-full px-4 py-2.5 bg-gray-900 border border-gray-600 text-gray-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-600 resize-none">{{ old('descripcion', $servicio->descripcion) }}</textarea>
            ...
            <div class="flex items-center gap-8 pt-1">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="requiere_repuestos" value="1"
                           {{ old('requiere_repuestos', $servicio->requiere_repuestos) ? 'checked' : '' }}
        ...
        <div class="mt-5 flex gap-3">
            <button type="submit"
                ...
                <i class="bi bi-check-lg" style="font-size:14px;"></i> Guardar cambios
            </button>
            <a href="{{ route('servicios.index') }}"
